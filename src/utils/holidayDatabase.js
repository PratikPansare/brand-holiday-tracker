// Built-in holiday database — 349 holidays across all 12 months
// Used as primary data source — instant, no network needed
...
export function getHolidaysForMonth(month, year) {
  const pad = String(month).padStart(2, "0")
  return ALL_HOLIDAYS
    .filter(h => h.date.startsWith(pad))
    .map(h => ({
      ...h,
      date: `${year}-${h.date}`,
      id: `h_${year}-${h.date}_${h.title.replace(/\W+/g, "_")}`,
      description: `${h.title} — ${h.category}. ${h.tags.split(",").slice(0,4).join(", ")}`,
    }))
}

export function getHolidaysForMonths(monthList) {
  return monthList.flatMap(({ month, year }) => getHolidaysForMonth(month, year))
}
